...
class ApacheAsyncClientInstrumentationModuleTest<REQUEST> extends AbstractHttpClientTest<REQUEST> {
...
//  }
  ...
  @Override
  public REQUEST buildRequest(String method, URI uri, Map headers) throws Exception {
    HttpPost postRequest = new HttpPost();
    postRequest.setHeader(headers.values().toArray()[0].toString(), headers.keySet().toArray()[0].toString());
    postRequest.setURI(uri);
    return (REQUEST) postRequest;
  }

  @Override
  public int sendRequest(Object o, String method, URI uri, Map headers) throws Exception {
    return client.execute((HttpUriRequest) o, new NoopFutureCallback()).get().getStatusLine().getStatusCode();
  }
